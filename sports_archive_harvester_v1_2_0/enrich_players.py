#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Enrich only essential player facts and one offline photo per player."""
    ...
    try:
        ...
        for index, row in enumerate(rows, 1):
            ...
            try:
                ...
                archive.conn.execute(
                    """UPDATE players SET date_of_birth=COALESCE(?,date_of_birth),date_of_death=?,nationality=COALESCE(?,nationality),place_of_birth=?,position=?,height_cm=?,weight_kg=?,gender=?,source_key=?,updated_at=? WHERE id=?""",
                    (birth, death, nationality, birthplace, position, height, weight,
                     item.get("strGender"), item.get("idPlayer"), now_iso(), row["id"]),
                )
                saved += 1
                log(
                    f"PLAYER ENRICHED | {index}/{len(rows)} | {name} | birth={birth or '?'} | nationality={nationality or '?'} | position={position or '?'}"
                )
                ...
                if team_name:
                    ...
                    if team_row:
                        cur = archive.conn.execute(
                            """INSERT OR IGNORE INTO team_memberships(team_id,player_id,shirt_number,position,start_date,is_current)
                               VALUES(?,?,?,?,?,1)""",
                            (int(team_row[0]), int(row["id"]), to_int(item.get("strNumber")), position, None),
                        )
        ...
        archive.conn.commit()
        log(
            f"PLAYER ENRICH COMPLETE | profiles={saved} | photos={photos} | memberships={memberships} | not_found={not_found} | errors={errors}"
        )
